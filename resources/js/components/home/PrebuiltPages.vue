...
const handleCategoryClick = (category: string) => {
  ...
  category == 'all' ? (currentItems.value = demoData) : (currentItems.value = demoData.filter((item) => item.category == category))
}
</script>

<template>
  <section id="pages" class="py-14 xxl:py-28 bg-n0 dark:bg-bg4">
    <div class="container">
      <div class="max-w-4xl mx-auto text-center mb-10 lg:mb-14">
        ...
        <p class="text-sm md:text-base">
          Dashboi ships with 94+ prebuilt pages, including dashboard and app examples, as well as collections like lists, grids, profile and personal pages and many other authentication and utility
          pages.
        </p>
      </div>
      <div class="grid grid-cols-12 gap-4 xxl:gap-6">
        <div class="max-md:hidden md:col-span-4 lg:col-span-3 xxl:col-span-2">
          <div class="flex flex-col sticky top-20 xxl:top-24 gap-1 xxxl:gap-2 bg-primary/5 px-3 py-4 rounded-xl border border-n30 dark:border-n500">
            <button
              v-for="category in categories"
              :key="category"
              @click="handleCategoryClick(category)"
              class="px-4 lg:px-6 py-2.5 xxl:py-3 rounded-xl duration-300 text-start text-sm lg:text-base capitalize font-medium"
        ...
        <div class="col-span-12 md:col-span-8 lg:col-span-9 xxl:col-span-10 grid grid-cols-2 gap-x-4 xxl:gap-x-6">
          <RouterLink
            v-for="{ id, img, title, desc, url } in currentItems.slice(0, 12)"
            :to="url"
            :key="id"
          ...
          >
            ...
            <div class="max-w-[90%] -translate-y-1/2 bg-n0 dark:bg-bg4 mx-auto rounded-xl border border-n30 dark:border-n500 py-4 px-6 xl:px-8">
              <h6 class="h6 mb-1 font-semibold">{{ title }}</h6>
              <p class="text-xs">{{ desc }}</p>
            </div>
          </RouterLink>
          <div class="flex justify-center col-span-2">
            <RouterLink to="/demos" class="btn-outline">
              View all 94 Demos
